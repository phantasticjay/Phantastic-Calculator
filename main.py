...
while True:
    ...
    if event == '=':
        if '+' in keys_entered:  # Finds if the calculator contains any input of the + sign
            keyList = keys_entered.split(
                '+')  # Splits the calculator input into a list by the + sign so only numbers remain
            add = 0
            ...
            keyList = keys_entered.split(
                'X')  # Splits the calculator input into a list by the X sign so only numbers remain
            print(keyList)
            multi = int(keyList[0])
            for i in range(1, len(keyList)):
